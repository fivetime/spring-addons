# 带登录、登出和 Authorities 映射的 Servlet OAuth2 Client
在本教程中，我们将一个 servlet（WebMVC）Spring Boot 3 应用配置为带有登录、登出和 authorities 映射的 OAuth2 client，并使用 OIDC Provider 上定义的角色实现 RBAC。
...
- 有一个 `ssl` profile，用于通过 SSL 运行应用，此时应用绑定的端口不是 `80` 或 `8080`（[原因是 servlet 应用中使用了 `PortMapperImpl`](https://github.com/spring-projects/spring-security/issues/12307)）
...
### 1.3. 静态首页
为了在认证后有内容可看，我们定义如下 `src/main/resources/static/index.html` 页面：
```html
<!DOCTYPE HTML>
<html>
...
```
现在可以运行应用并访问 [http://localhost:8080](http://localhost:8080)。
...
Spring 为实现了 [RP-Initiated Logout](https://openid.net/specs/openid-connect-rpinitiated-1_0.html) 的 OIDC Provider 提供了 `LogoutSuccessHandler`：`OidcClientInitiatedLogoutSuccessHandler`。但要覆盖 security filter chain 的部分配置，我们必须提供一个完整的 `SecurityFilterChain` bean。

### 2.1. 标准 RP-Initiated Logout
需要对用户进行认证的应用，其 OAuth2 client security filter chain 应包含：
- 登录
- 登出
- session（默认启用）
- CSRF 保护（因为有 session，且使用 session repository 时默认启用）
- 允许未授权用户至少访问登录和 authorization-code 流程所需资源的访问控制
...
我们来自己编写一个 `LogoutSuccessHandler`，用于指定登出 URI 以及登出后跳转 URI 的参数名。

首先，在 Boot 主类上添加 `@ConfigurationPropertiesScan`，然后声明配置属性：
```java
@Data
@Configuration
...
```
将对应属性添加到 yaml 中（注意 provider ID 必须与 `spring.security.oauth2.client.registration` 下的 ID 一致）：
```yaml
logout:
  registration:
    cognito-confidential-user:
...
```
现在，我们可以定义一个 logout success handler，解析此配置用于非标准 RP-Initiated Logout（参考 Spring 的 `OidcClientInitiatedLogoutSuccessHandler` 实现）：
```java
...
```
此 handler 根据配置属性在 Spring 的 `OidcClientInitiatedLogoutSuccessHandler` 和我们的 `AlmostOidcClientInitiatedLogoutSuccessHandler` 之间切换。
...
根据[官方文档](https://docs.spring.io/spring-security/reference/servlet/oauth2/login/advanced.html#oauth2login-advanced-map-authorities)，有两种选择：
- 提供一个 `GrantedAuthoritiesMapper` bean
- 提供并配置一个 `OAuth2UserService<OidcUserRequest, OidcUser>`
...
```
现在，只有在[前置条件](https://github.com/fivetime/spring-addons/tree/master/samples/tutorials#2-prerequisites)配置 OIDC Provider 时授予了 `NICE` 角色的用户，才能访问 [http://localhost:8080/nice.html](http://localhost:8080/nice.html) 页面（启用 `ssl` profile 时为 [https://localhost:7443/nice.html](https://localhost:7443/nice.html)）。
...
由于绕过 `InMemoryAuthorizedClientRepository` 相当复杂，我们接下来要实现一个防护机制，阻止已认证的用户再次登录：用户必须先登出才能重新登录。
...
第一条规则很容易实现：将 `/` 添加到 `permitAll()` 列表中。但第二条规则需要在登录页过滤器之前插入一个过滤器（有一个特殊的"登录页"过滤器会绕过请求授权）：
```java
...
但且慢，我们在这里所做的一切相当冗长，而几乎每个 OAuth2 client 都会用到这些配置。难道每次都要重复写这些吗？其实不必：本仓库提供了 [`spring-addons-webmvc-client`](https://github.com/fivetime/spring-addons/tree/master/webmvc/spring-addons-webmvc-client) Spring Boot starter 正是为此而生。如果出于某种原因不想使用它，也可以编写[自己的 starter](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.developing-auto-configuration)，将我们在这里编写的配置封装起来。

另外，如果我们确实需要用户同时持有多个 authorized client（例如同时在 Google 和 Facebook 上认证，以从同一个 client 查询 Google API 和 Facebook Graph），可以像上一节建议的那样提供一个自定义的 `OAuth2AuthorizedClientRepository`。本仓库的 client starter 提供了这样的实现，它在用户 session 中按 issuer 存储认证信息，并在尝试检索 authorized client 前解析正确的认证（及其 subject）。同样，如果不想使用这些 starter，也可以[自己实现](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.developing-auto-configuration)。
